...
public class XSPopupMenu extends JFrame {
	...
	private XSMenuItemListener menuItemListener = new XSMenuItemListener() {
		...
		public void mouseEntered(XSMenuItem item) {
			...
			if (item.countChildren(XSMenuItem.MenuType.PRIMARY) > 0) {
		...
		public void mouseActionClicked(XSMenuItem item, int buttonIndex) {	
			...
			switch (buttonIndex) {
				...
				case MouseEvent.BUTTON3 -> {
					...
					if (item.countChildren(XSMenuItem.MenuType.AUXILIARY) > 0) {
						Point menuLocation = MouseInfo.getPointerInfo().getLocation();
						mustOpenSubMenu(item, XSMenuItem.MenuType.AUXILIARY, menuLocation);
					}
				}
	...
	private void scheduleDelayedOpenSubMenu(final XSMenuItem menuItem, final Point menuLocation) {
		synchronized (subMenuFutureSync) {
			cancelDelayedOpenSubMenu();
			subMenuFuture = Resources.xsGuiPlatform.getPlatformResource(XSPlatformResource.TASK_SCHEDULER).schedule(() -> {
				SwingUtilities.invokeLater(() -> {
					mustOpenSubMenu(menuItem, XSMenuItem.MenuType.PRIMARY, menuLocation);
				});
	...
	private void mustOpenSubMenu(XSMenuItem menuItem, XSMenuItem.MenuType menuType, Point menuLocation) {
		cancelDelayedOpenSubMenu();
		currentSubMenu = new XSPopupMenu(this, menuItem, menuType);
		currentSubMenu.addWindowListener(subMenuWindowEventsHandler);
		currentSubMenu.show(menuLocation.x, menuLocation.y);
	}
	
	public XSPopupMenu() {
		this(null, null, XSMenuItem.MenuType.PRIMARY);
	}
	
	public XSPopupMenu(XSPopupMenu parentMenu, XSMenuItem menuSource, XSMenuItem.MenuType menuType) {
		...
		int itemCount = menuSource.countChildren(menuType);
		
		for(int i=0; i<itemCount; i++) {
			XSMenuItem menuItem = menuSource.getChild(menuType, i);
